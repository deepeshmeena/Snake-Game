...
(define scn (place-image (rectangle 400 50 "outline" "black") 200 425 (empty-scene 400 450)))
...
(define game2 (make-game (make-snake (make-posn 110 150)
                         (list (make-posn 110 160) (make-posn 110 170))
                         "up")
                         (make-posn 310 310)))
...
(define (render-food g)
  (place-image (text (number->string score) 25 "green") 170 425
               (place-image (text "Score: " 25 "green") 110 425
...
(define (render-snake w image)
  (cond [(hits-wall? w) (place-image (text (number->string score) 25 "green") 170 425
                                     (place-image (text "Score: " 25 "green") 110 425
                            (place-image (text (number->string score) 25 "green") 260 50
                                     (place-image (text "Your Score: " 25 "green") 160 50
                                                  (place-image (text "snake hits wall" 20 "black")
                                  100 300 scn)))))]
        [(hits-itself? w (snake-tail w)) (place-image (text (number->string score) 25 "green") 170 425
                                                      (place-image (text "Score: " 25 "green") 110 425
                                                                   (place-image (text (number->string score) 25 "green") 260 50
                                                                                (place-image (text "Your Score: " 25 "green") 160 50
                                                                                             (place-image (text "snake hits itself" 20 "black")
                                                                                                          100 300 scn)))))] 
        [(cons? (snake-tail w)) (render-head (snake-head w) (render-tail (snake-tail w) image))]))
...
;only changes the direction of the snake 
...
(define (new-food-pos w)
  (let* [(x (* 10 (+ (* 2 (random 20))1))) ;390
         (y (* 10 (+ (* 2 (random 20))1)))] ;390
    (if (not-overlap (make-posn x y) w)
        (make-posn x y)
        (new-food-pos w))))
;checks if food overlaps with the snake
...
(define (lengthen g) 
  (make-game (make-snake (make-new-head (game-snake g))
                        ...
                        ;add-to-tail is for updating the snake when it eats food just appends a new head to the old snake.
...
(define (game-over g)
  (if (or (hits-wall? (game-snake g))
      (hits-itself? (game-snake g) (snake-tail (game-snake g)))) true false))


(big-bang game2 
          [to-draw render-game]
          [on-tick tick-game 0.2]
          [on-key key-game]
          [stop-when game-over]
          )